...
kab ships three of this skill's four intent files -- date_trivia.intent,
number_trivia.intent and year_trivia.intent -- beside math.voc, random.voc
and year.voc, and it is covered here with three rows each. The earlier text
said kab shipped "no .intent files at all" and was excluded for having no
match path; read from the tree, that was false, and the nine kab rows below
are the refutation: kab routes.

kab's one real gap is math_trivia.intent, which it does not ship. Every
intent in this skill is a file-intent, so math.voc alone gives that handler
nothing to match.
...
def _stop_all_minicrofts():
    yield
    # Stop in reverse creation order. Each MiniCroft saves the `lang` that was
    # current when IT was constructed and writes that value back on stop, so
    # the saves nest: the en-US croft holds the process default, the it-IT
    # croft holds "en-US", the nl-NL croft holds "it-IT", and so on. Stopping
    # in creation order unwinds that stack backwards and the last stop leaves
    # the process on a foreign locale, which every later test file in the same
    # xdist worker then inherits. LIFO is the only order that restores the
    # value each croft actually saved.
    for mc in reversed(list(_MINICROFTS.values())):
        mc.stop()
    _MINICROFTS.clear()
