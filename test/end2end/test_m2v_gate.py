    ...
    def _run(self, utterance: str, lang: str = LANG, timeout: float = 15.0):
        ...
        try:
            ...
            while _t.time() < deadline and not speaks and not failures:
                _t.sleep(0.05)
            # A handler with no extractable number speaks no_number_found
            # before the fact, so the fact is not always the first line.
            # Give the second speak a moment to land.
            settle = _t.time() + 1.0
            while _t.time() < settle:
                _t.sleep(0.05)
        finally:
        ...
        return [(m.data.get("utterance") or "") for m in speaks], False

    def _assert_effect(self, utterance, expected_fact):
        spoken, failed = self._run(utterance)
        self.assertFalse(failed, f"{utterance!r} did not route: complete_intent_failure")
        self.assertTrue(spoken, f"{utterance!r} produced no spoken output")
        self.assertIn(
            expected_fact, spoken,
            f"{utterance!r} spoke {spoken!r}, expected the stubbed fact {expected_fact!r}")
